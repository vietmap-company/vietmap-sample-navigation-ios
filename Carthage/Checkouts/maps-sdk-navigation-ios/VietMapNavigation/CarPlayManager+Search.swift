...
extension CarPlayManager: CPSearchTemplateDelegate {
    ...
    public func searchTemplate(_ searchTemplate: CPSearchTemplate, updatedSearchText searchText: String, completionHandler: @escaping ([CPListItem]) -> Void) {
        let title = NSLocalizedString("CARPLAY_SEARCH_NOT_IMPLEMENTED", bundle: .VietMapNavigation, value: "Search not implemented", comment: "Title in search template when search is unimplemented")
    ...
    static func resultsOrNoResults(_ items: [CPListItem], limit: UInt? = nil) -> [CPListItem] {
        ...
        if items.count > 0 {
            ...
            let title = NSLocalizedString("CARPLAY_SEARCH_NO_RESULTS", bundle: .VietMapNavigation, value: "No results", comment: "Message when search returned zero results in CarPlay")
