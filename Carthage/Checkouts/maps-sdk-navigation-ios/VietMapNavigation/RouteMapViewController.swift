...
class RouteMapViewController: UIViewController {
    ...
    lazy var endOfRouteViewController: EndOfRouteViewController = {
        let storyboard = UIStoryboard(name: "Navigation", bundle: .VietMapNavigation)
...
//        mapView.trackUpdateCourse = false
    ...
    @objc func willReroute(notification: NSNotification) {
        let title = NSLocalizedString("REROUTING", bundle: .VietMapNavigation, value: "Rerouting…", comment: "Indicates that rerouting is in progress")
        lanesView.hide()
    ...
    func notifyUserAboutLowVolume() {
        ...
        let title = String.localizedStringWithFormat(NSLocalizedString("DEVICE_VOLUME_LOW", bundle: .VietMapNavigation, value: "%@ Volume Low", comment: "Format string for indicating the device volume is low; 1 = device model"), UIDevice.current.model)
    ...
    @objc func didReroute(notification: NSNotification) {
        ...
        if notification.userInfo![RouteControllerNotificationUserInfoKey.isProactiveKey] as! Bool {
            let title = NSLocalizedString("FASTER_ROUTE_FOUND", bundle: .VietMapNavigation, value: "Faster Route Found", comment: "Indicates a faster route was found")
